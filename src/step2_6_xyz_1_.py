...
from make_6_xyz_0 import exec_gjf##計算した点のxyzfileを出す
...
def main_process(args):
    ...
    if not os.path.exists(auto_csv_path):        
        df_E = pd.DataFrame(columns = ['a','b','theta','A2','Rt','E','E_p','E_t1','machine_type','status','file_name'])##いじる
...
def listen(auto_dir,monomer_name,num_nodes,num_m2,isTest):##args自体を引数に取るか中身をばらして取るかの違い
    ...
    for idx,row in zip(df_queue.index,df_queue.values):
        ...
        if len(E_list)!=2:##get Eの長さは計算した分子の数
            ...
            Et1=float(E_list[0]);Ep=float(E_list[1])##8分子に向けてep1,ep2作成　ep1:b ep2:a
            E = 2*(Et1+Et1+Ep)##エネルギーの値も変える
            df_E.loc[idx, ['E_t1','E_p','E','status']] = [Et1,Ep,E,'Done']
    ...
    if isAvailable:
        ...
        if len(params_dict)!=0:#終わりがまだ見えないなら
            ...
            if not(alreadyCalculated):
                ...
                df_newline = pd.Series({**params_dict,'E':0.,'E_p':0.,'E_t1':0.,'machine_type':machine_type,'status':'InProgress','file_name':file_name})
...
def get_opt_params_dict(df_cur, init_params_dict,fixed_params_dict):
    ...
    A2 = init_params_dict['A2']; Rt = init_params_dict['Rt']
    while True:
        E_list=[];ab_list=[]
